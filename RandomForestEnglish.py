...
if __name__ == '__main__':
    ...
    X, Xt, Y, Yt = train_test_split(data['text'], data['sentiment'], test_size=0.2)

    vectorizer = TfidfVectorizer()
    train_tokenized = vectorizer.fit_transform(X)
    test_tokenized = vectorizer.transform(Xt)
    ...
    forest.fit(train_tokenized, Y)
    #print(forest.predict(test_tokenized))
    print("Train score: ", forest.score(train_tokenized, Y))
    print("Test score: ", forest.score(test_tokenized, Yt))
    ...
    plt.show()
    print(classification_report(Yt, forest.predict(test_tokenized)))
